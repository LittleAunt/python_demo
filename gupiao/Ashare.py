...
def get_price_day_tx(code, end_date="", count=10, frequency="1d"):  # 日线获取
    ...
    # print(f"#############\n{buf}")
    # 移除分红那天的数据后的分红数据，避免数据解析问题
    # ['2015-07-27', '25.294', '21.964', '25.484', '21.794', '1712595.000', {
	# 	'nd': '2014',
	# 	'fh_sh': '4.75',
	# 	'djr': '2015-07-24',
	# 	'cqr': '2015-07-27',
	# 	'FHcontent': '10派5元转10股'
	# }]
    cleaned_data = [[item for item in sublist if not isinstance(item, dict)] for sublist in buf]
    # print(f"##### 移除后的 {cleaned_data}")
    df = pd.DataFrame(cleaned_data, columns=["time", "open", "close", "high", "low", "volume"])
...
def get_price(code, end_date="", count=10, frequency="1d", tx_channel=True, fields=[]):  # 对外暴露只有唯一函数，这样对用户才是最友好的
    xcode = code.replace(".XSHG", "").replace(".XSHE", "")  # 证券代码编码兼容处理
    xcode = ("sh" + xcode if ("XSHG" in code) else "sz" + xcode if ("XSHE" in code) else code)

    if frequency in ["1d", "1w", "1M"]:  # 1d日线  1w周线  1M月线
        if (tx_channel): # 腾讯接口是前复权数据
