...
QH_CODE = "233773" # 期货代码
START_DATE = "2023-12-11" # 模拟买卖的起始日期 2023-12-13
END_DATE = "2024-03-22" # 结束日期
BUY_COUNT = 1 # 购买的手数

start_date = pd.to_datetime(START_DATE) # 将字符串日期转换为 datetime 类型
end_date = pd.to_datetime(END_DATE)
days_before = 200 # 起始天数往前多取 120 天，用于计算 MACD 指标
date_before = start_date - timedelta(days=days_before)
df = get_price_day(QH_CODE, date_before.strftime('%Y-%m-%d'), END_DATE)
# 过滤想要的日期时间段数据
...
def cal_profit(last_price, cur_price, duo):
...
# 添加一条交易记录
def add_record(date, price, type):
    # 列表为空，直接插入一条交易记录
    if len(records) == 0:
        ...
        if type == TYPE_D:
            if last_record[RECORD_TYPE] == TYPE_K: # 之前是做空，计算收益
                records.append({RECORD_DATE: date, RECORD_PRICE: price, RECORD_TYPE: type, RECORD_PROFIT: cal_profit(last_record[RECORD_PRICE], price, False)})
            elif last_record[RECORD_TYPE] == TYPE_P: # 之前是平仓，直接插入一条新交易
                records.append({RECORD_DATE: date, RECORD_PRICE: price, RECORD_TYPE: type, RECORD_PROFIT: 0})
        # 2. 当前是做空
        elif type == TYPE_K:
            if last_record[RECORD_TYPE] == TYPE_D: # 之前是做多，计算收益
                records.append({RECORD_DATE: date, RECORD_PRICE: price, RECORD_TYPE: type, RECORD_PROFIT: cal_profit(last_record[RECORD_PRICE], price, True)})
            elif last_record[RECORD_TYPE] == TYPE_P: # 之前是平仓，直接插入一条新交易
                records.append({RECORD_DATE: date, RECORD_PRICE: price, RECORD_TYPE: type, RECORD_PROFIT: 0})
...
# 取最后一笔交易如果没平仓，直接平仓
last_row = df.iloc[-1]
if df.index[-1] != records[-1][RECORD_DATE]:
    add_record(df.index[-1], last_row.close, TYPE_P)
# 总收益
