...
while not is_bet:
    # im 平台
    on_game_list = ffBC.crawling()
    if on_game_list == None:
        bc_print.print_red("188 数据获取失败！")
        time.sleep(30)
        continue
    print("**********************************************************************")
    print(f"188 平台比赛个数：{len(on_game_list)}")
    time.sleep(30)
    continue
    # panda 平台
    ...
    for match_result in match_result_list:
        ...
        if is_bet_money_a:
            ...
            bet_money_b = int(bet_money_a * (ratio_a + 1) /
                              (ratio_b + 1)) - REDUCE_MONEY
        else:
            bet_money_b = bet_money
            bet_money_a = int(bet_money_b * (ratio_b + 1) /
                              (ratio_a + 1)) - REDUCE_MONEY
        print(
        ...
        if cmd.strip() == "y":
            ...
            if zd == "game_a":
                ...
                iszd_a = False
            is_ok_a = oneNineBC.check_bet(
                game_a, pk, bet, iszd_a, ratio_a, bet_money_a)
            if is_ok_a:
            ...
            if zd == "game_b":
                ...
                iszd_b = False
            is_ok_b = pandaBC.check_bet(
                game_b, pk, bet, iszd_b, ratio_b, bet_money_b)
            if is_ok_b:
